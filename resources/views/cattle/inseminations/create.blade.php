<x-dlayout>
    <div class="p-6">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 max-w-2xl">
            <h3 class="text-lg font-semibold text-gray-900 mb-6">Add Insemination Record</h3>
            <form action="{{ route('inseminations.store') }}" method="POST">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <select name="cow_id" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" required>
                            ...
                            @foreach($cattle as $cow)
                                <option value="{{ $cow->id }}" @selected(old('cow_id') == $cow->id)>{{ $cow->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="bull_number" value="{{ old('bull_number') }}"
                               class="w-full rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500" required>
                        @error('bull_number')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        ...
                        <select name="successful" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                            ...
                            <option value="1" @selected(old('successful') === '1')>Successful</option>
                            <option value="0" @selected(old('successful') === '0')>Failed</option>
                        </select>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Expected Date of Birth <span class="text-gray-400 text-xs">(if successful)</span></label>
                        <input type="date" name="expected_dob" value="{{ old('expected_dob') }}"
                               class="w-full rounded-xl border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                        @error('expected_dob')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
